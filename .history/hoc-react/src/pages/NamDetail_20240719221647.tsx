...
const getProductDetail = (id: number): ProductNam | undefined => {
    return nam.find(product => product.id === id)
}

const ProductLinePageDetail: React.FC = () => {
    const {id} = useParams<{id?:string}>();
    const productId = id ? parseInt(id,20) : undefined;
    const product = productId !== undefined ? getProductDetail(productId):undefined;
    ...
    return (
        <div className="mt-32 mb-5  w-[1120px]  flex justify-around   m-auto leading-7">
            <img src={product.img} alt={product.infomation} className="w-[575px] h-[575px]" />
            <div className="mt-[35px] leading-8">
                <h2 className="text-[#383838] text-[27px] font-[600]">{product.infomation}</h2>
...
export default ProductLinePageDetail